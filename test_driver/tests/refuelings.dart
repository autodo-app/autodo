...
Future<void> editRefueling(FlutterDriver driver) async {
  // edit button
  print('scrolling');
  await driver.scrollUntilVisible(find.byValueKey('__refuelings_screen_scroller__'), find.byValueKey('__refueling_card_edit_test_2000'), dyScroll: -100.0);
  print('tapping');
  await driver.tap(find.byValueKey('__refueling_card_edit_test_2000'));
...
Future<void> deleteRefueling(FlutterDriver driver) async {
  // press button
  print('scrolling');
  await driver.scrollUntilVisible(find.byValueKey('__refuelings_screen_scroller__'), find.byValueKey('__refueling_delete_button_test_3000'), dyScroll: -100.0);
  print('tapping');
  await driver.tap(find.byValueKey('__refueling_delete_button_test_3000'));

  // verify that the card no longer exists
  await driver.waitForAbsent(find.byValueKey('__refueling_card_edit_test_3000'));
}